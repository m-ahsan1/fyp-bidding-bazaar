...
const AuctionDetailPage = () => {
    ...
    const isCurrentUserWinner = auth.currentUser && auth.currentUser.email === curWinner;
    const isCurrentUserSeller = auth.currentUser && auth.currentUser.email === email;
    ...
    const handleBid = () => {
        ...
        const newPrice = Math.floor((bidPrice / 100) * 110);
        bidAuction(id, newPrice);
    };
            ...
            <div className="w-full md:w-1/2 bg-white p-8 rounded-lg shadow-lg">
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <p className="text-gray-600"><strong>Manufacturer:</strong> {manufacturer}</p>
                        <p className="text-gray-600"><strong>Model Year:</strong> {modelYear}</p>
                        <p className="text-gray-600"><strong>Mileage:</strong> {mileage} km</p>
                        <p className="text-gray-600"><strong>Starting Price:</strong> ${price}</p>
                        {curPrice !== undefined && (
                            <p className="text-gray-600"><strong>Current Price:</strong> ${curPrice}</p>
                        )}
                        {isCurrentUserWinner ? (
                            <p className="text-gray-600"><strong>You are the highest bidder</strong></p>
                        ) : isCurrentUserSeller && !isAuctionEnded ? (
                            <button className="btn btn-primary" onClick={() => { endAuction(id) }}>End Auction</button>
                        ) : null}
                    </div>
                    <Countdown
                        date={duration}
