...
def predict_price(input_data, model_path='xgb_regressor_model.pkl'):
    ...
    predicted_price = max(0, float(predicted_price[0]))  # Cast to float
...
def predict():
    try:
        ...
        exterior = form_data['exterior']
        engine = form_data['engine']
        suspension = form_data['suspension']
        interior = form_data['interior']
        heater = form_data['heater']
        ...
        data = {
        'rating': int(rating),
        'exterior & body': int(exterior),
        'engine/transmission/clutch': int(engine),
        'suspension/steering': int(suspension),
        'interior': int(interior),
        'ac/heater': int(heater),
        'company': int(companydic_str[companyname]),
        'variant': int(varientdic_str[variant]),
        'fuel type': int(fueldic[fuel]),
        'model date': int(modelyear),
        'mileage': int(mileage)
        }
